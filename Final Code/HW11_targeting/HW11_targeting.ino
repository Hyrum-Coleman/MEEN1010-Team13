...
void setup(void)
{
  ...
  double xTarget = 1.2;
  double xTargetVec[] = {0.7, 0.8, 0.9, 1.0, 1.1, 1.2};
  double H[] = {.1301, .0960, .0880, .0465};
  double alpha = 38.4;
  double beta = .015;
  double thetaL0 = 11.77;
  double rangeAngle = RangeAngle(d, k, lambda);
  double range = LandingDistance(d, k, lambda, rangeAngle);
  double thetaL = LaunchAngle(d, k, lambda, xTarget);
  double thetaS = ThetaServo(H, thetaL, alpha, beta, thetaL0);
  ...
  if (xTarget <= range) {
    ...
    Serial.print(" use a launch angle of ");
    Serial.print(thetaL);
  ...
  Serial.print(thetaL, 2);
  Serial.print(" deg, use ThetaS = ");
  Serial.print(thetaS, 2);
  Serial.println(" deg");
  Serial.println("");
